...
Suppose we have $S$ strata and the $s$th stratum contains $N_s$ units, $n_s$ of which are treated. Then there are 
$$\prod_{s=1}^S {N_s \choose n_s}$$
...
On the other hand, if the null hypothesis is false, then the residuals will capture some of the effect of treatment:
$$ Y_i - \hat{Y}_i \not\independent T_i$$

We will use the average difference in means across strata as our test statistic:

$$\tau(Y, T) = \frac{N_s}{N} \sum_{s=1}^S \left\lvert \frac{n_s}{N_s} \sum_{i \in s : T_i=1} \left(Y_i - \hat{Y}_i \right) - \frac{N_s - n_s}{N_s} \sum_{i \in s: T_i=0} \left(Y_i - \hat{Y}_i \right) \right\rvert$$
\todo{keep notation straight. s can't be index and set}
}
